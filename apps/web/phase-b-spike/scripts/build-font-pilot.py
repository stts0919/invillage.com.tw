#!/usr/bin/env python3
"""Build the local /spaces-only OFL font pilot with a pinned isolated toolchain.

Run with Python 3.12 + fonttools 4.66.0 + Brotli 1.2.0. Inputs are never edited.
The generated binaries live in git-ignored assets/optimized/ and public/assets/.
"""
...
def rename_and_license(font: TTFont, family: str, license_text: str) -> None:
...
def build_font(kind: str, source: Path, license_path: Path, texts: list[str]) -> dict[str, object]:
    spec = SOURCES[kind]
    ...
    try:
        ...
        if kind == "editorial":
            font = instantiateVariableFont(font, {"wght": 700}, inplace=True, downgradeCFF2=True)
        if "fvar" in font:
            raise ValueError(f"{kind}: expected static 700-weight subset")
        rename_and_license(font, spec["family"], license_text)
        font.flavor = "woff2"
        OPTIMIZED.mkdir(parents=True, exist_ok=True)
        output = OPTIMIZED / spec["filename"]
        # A per-build staging file keeps a failed save from replacing a reviewed output.
        staged = OPTIMIZED / (spec["filename"] + ".building")
        if staged.exists():
    ...
    for destination in (PUBLIC / spec["filename"], PUBLIC / spec["license_filename"], OPTIMIZED / spec["license_filename"]):
    ...
    result = TTFont(OPTIMIZED / spec["filename"])
    try:
        output_missing = sorted(character for character in characters if ord(character) not in result.getBestCmap())
        if output_missing or result.flavor != "woff2" or result["OS/2"].usWeightClass != 700:
            raise ValueError(f"{kind}: output format, weight or coverage invalid: {output_missing}")
        if ("fvar" in result or names(result, 1) != [spec["family"]]
                or names(result, 4) != [spec["family"] + " Bold"]
                or names(result, 6) != [spec["family"].replace(" ", "") + "-Bold"]
                or names(result, 16) != [spec["family"]]):
    ...
    return {
        ...
        "publicPath": str((PUBLIC / spec["filename"]).relative_to(REPO)),
        "licensePublicPath": str((PUBLIC / spec["license_filename"]).relative_to(REPO)),
...
def main() -> None:
    ...
    parser.add_argument("--stone-license", type=Path, required=True)
    ...
    print(json.dumps({"schemaVersion": 1, "scope": "/spaces local pilot", "toolVersions": {"fonttools": "4.66.0", "Brotli": "1.2.0"}, "fonts": results}, ensure_ascii=False, indent=2))
